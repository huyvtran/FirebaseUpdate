...
//                         data={item}
//                         width={wth}
//                         height={height} flex={flex} textStyle={textStyle}
//                         {...props}
//                     />;
//                 })}
//             </View>
...
//     }
// }
export const Row = ({ data, style, widthArr, height, flexArr, textStyle, onPressCell }) => {
    let width = widthArr ? sum(widthArr) : 0;
    ...
    return data ? (
        <View style={[height && { height }, width && { width }, styles.row, style]}>
            {data.map((item, i) => {
                ...
                const wth = widthArr && widthArr[i];
                return <Cell
                    onPress={() => onPressCell && onPressCell(item, i)}
                    key={i}
                    data={item}
                    width={wth}
                    height={height} flex={flex} textStyle={textStyle}
